...
def main() -> int:
    ...
    server = start_server(OUT_DIR / "vllm_server.log", port=VLLM_PORT, mem_util=0.45, device="1")
    try:
        ...
        log(f"real rollouts: {len(gens)} GenerationEvents (64 prompts x 8 gens)")
        ...
        (OUT_DIR / "batch_online_matrix.json").write_text(json.dumps({
            "run_id": run_id,
            "scope": "F1-F8 batch online matrix (512 real rollouts, 64 prompts x 8 gens, per-generation injection)",
