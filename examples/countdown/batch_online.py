...
One GPU session:
  1. 128 REAL rollouts (32 prompts x 4 gens) via trl vllm-serve;
...
VLLM_PORT = int(os.environ.get("GRPO_GUARD_VLLM_PORT", "8005"))
GROUP_PORT = int(os.environ.get("GRPO_GUARD_GROUP_PORT", "51220"))
...
def main() -> int:
    ...
    try:
        ...
        log(f"real rollouts: {len(gens)} GenerationEvents (32 prompts x 4 gens)")
        ...
        (OUT_DIR / "batch_online_matrix.json").write_text(json.dumps({
            "run_id": run_id,
            "scope": "F1-F8 batch online matrix (128 real rollouts, 32 prompts x 4 gens, per-generation injection)",
