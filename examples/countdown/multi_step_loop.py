"""Multi-step guarded closed loop (decision D14): v0 -> v1 -> v2 -> v3.

Three consecutive committed update-sync-rollout cycles on the REAL server
(trl vllm-serve + Qwen3-4B), each with canary check, so the optimizer's
weights genuinely move across steps and the sync path is exercised 3x.

    COMMITTED(v{k-1}) -> CANARY(v{k-1}) -> GENERATION(v{k-1})
      -> IDENTITY(allow) -> REWARD -> PRE_UPDATE(allow)
      -> MATERIALIZED -> UPDATE_COMMITTED(v{k}) -> SYNC -> CANARY(v{k})

Plus a near-max-context rollout (~1900 tokens) at the end to stress the
mask/span reconstruction at the boundary.

Honest reporting: loss is expected ~0 (each step consumes its own policy's
trajectories, ratio ~ 1); the real new evidence is (a) 3 committed
updates with observable weight deltas, (b) 3 observed syncs + canaries,
(c) the long-context boundary rollout.  All numbers measured, none
fabricated (weight deltas computed at fp32; if 0, reported as 0).

Outputs: <out>/multi_step.json
"""
...
def main() -> int:
    ...
    try:
        ...
        for k in range(1, N_STEPS + 1):
            log(f"--- step {k}: rollout v{behavior_version} ---")
            ...
            for p in prompts:
                for g in range(N_GENS):
                    ...
                    env_id = build_envelope(run_id, gen, None, None,
                                            ...
                                            split_manifest, "pre_reward", behavior_version,
                                            f"update-{k}")
            ...
            log(f"step {k} rollout: {len(identity_events)} sequences, identity ALLOW on all")

            handles = []
            for (gen, id_decision, env_id, _), rew in zip(identity_events, reward_events):
                pre = build_envelope(run_id, gen, rew, id_decision,
                                     ckpt_prev["checkpoint_manifest_sha256"],
                                     split_manifest, "pre_update", behavior_version,
            ...
            steps.append({
                ...
                "checkpoint_manifest_sha256": ckpt_k["checkpoint_manifest_sha256"],
                "rollout_sequences": len(identity_events),
            ...
            log(f"step {k} done: ||dθ||(fp32 vs v0)={delta_vs_v0:.6f}")
        ...
        result = {
            "run_id": run_id,
            "scope": "multi-step guarded closed loop (D14): 3 committed update-sync-rollout "
                     "cycles + near-max-context boundary rollout, real server (Qwen3-4B)",
            "n_steps": N_STEPS, "prompts_per_step": N_PROMPTS, "gens_per_prompt": N_GENS,
            "lr": LR, "canary_calibration_reloads": 5, "canary_tolerance": tolerance,
            "steps": steps,
            ...
            "summary": {
                ...
                "max_weight_delta_fp32_vs_v0": max(s["weight_delta_fp32_vs_v0"] for s in steps),
                "final_weight_delta_fp32_vs_v0": steps[-1]["weight_delta_fp32_vs_v0"],
                "losses": [s["loss"] for s in steps],
            },
        }
        (OUT_DIR / "multi_step.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
        log(f"MULTI-STEP DONE: committed={len(steps)} canaries={result['summary']['all_canaries_pass']} "
            f"final||dθ||={steps[-1]['weight_delta_fp32_vs_v0']:.6f}")
        return 0
    finally:
        stop_server(server)
